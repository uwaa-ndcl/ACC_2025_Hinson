...
load(['./Data/q' num2str(systems(zz)) '/' datasets(seed).name]);

[z,p,k] = butter(5,.5/25,'high');                                           % High pass filter accelerometer data to remove drift
[b,a] = zp2tf(z,p,k);

tt = t;
y_noise = c_scale*[y(1,:)-mean(y(1,:)); y(2,:)-mean(y(2,:));filtfilt(b,a,y(3,:)-mean(y(3,:)))];
